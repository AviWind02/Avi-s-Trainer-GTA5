...
namespace feature {
    namespace vehiclespawner {
        ...
        void VehicleSpawnView() {//Left this view here for now Move into View folder later
            ...
            for (int i = 0; i < g_vehicleCategories.size(); ++i) {
                if (GUI::buttons::Sub(g_vehicleCategories[i].c_str(), pages::VehicleSpawner_Picked_page)) {
                    selectedClass = i;
                }
            }
        }
        ...
        void PickedVehicleView() {
            ...
            GUI::menu::SetSubTitle(g_vehicleCategories[selectedClass].c_str());
            ...
            for (const auto& vehicle : g_vehicleList) {
                if (selectedClass == 23) {
